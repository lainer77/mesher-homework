...
테스트 케이스를 추리기 위해 기능에 이상이 없는지 전체적으로 검토하고, 리팩토링을 진행하고, 테스트 케이스 리스트를 추리는 단계입니다.

1. 기본플로우 테스트
    - 첫번째 토큰값 변경 1 -> 첫번째 달러표시 -> 두번째 토큰값 변경 -> 두번쨰 달러표시 -> 1코인 값어치 표기 -> 액션 버튼 활성화 -> 스왑
2. 스왑 테스트
    - 첫번째 토큰값 변경 2 -> 화살표 버튼 클릭 -> 첫번째, 두번째 변경 -> 화살표 버튼 클릭 -> 원상복구
3. 셀렉션박스 모달 테스트
    - 첫번쨰 셀렉션박스 클릭 -> 모달 열림 -> 심볼검색 AXS -> AXS 클릭 -> 모달 닫힘 -> 첫번째 AXS로 변경 -> 토큰값 변경에 따른 -> 첫번째 달러표시 -> 두번째 토큰값 변경 -> 두번쨰 달러표시
4. 입력값 삭제 테스트
    - 첫번째 토큰값 변경 1 -> 두번째 토큰 값 확인 -> 두번째 토큰 값 삭제 -> 첫번째 토큰 값 삭제 확인 -> 1코인 값어치 표기 사라짐 -> 액션 버튼 비활성화 -> 금액을 입력하세요
5. 모달 테스트
    - 첫번쨰 셀렉션박스 클릭 -> 모달 열림 -> 모달 닫기 버튼 클릭 -> 모달 닫힘 -> 두번째 셀렉션 박스 클릭 -> 모달 열림 -> 모달 닫기 버튼 클릭 -> 모달 닫힘
6. 최근 토큰 목록 테스트
    - 로컬스토리지 클리어 -> 첫번쨰 셀렉션박스 클릭 -> 모달 열림 -> 최근 토큰 목록 비어있음 -> ETH 클릭 -> 모달 닫힘 -> 첫번쨰 셀렉션박스 클릭 -> 모달 열림 -> ETH 추가됨 확인

## 테스트 코드 작성

1. gpt로 작성한 플로우에 맞춰서 초안을 작성하게 만들고, 빠진 부분을 보충하였습니다.
2. 각 컴포넌트에 테스트 코드가 엘레멘트를 찾을 수 있도록 testid를 심어주는 작업을 진행.
3. 실제 플로우대로 동작하도록 마무리작업
4. 특이사항으로 하프모달은 투명값이 적용되어 있어서 실제 사라지는게 아니라서 css의 값을 추적해서 1이면 나타난거고, 0이면 사라진 것으로 인식하게 작업했습니다. 애니메이션 작업을 위해 이렇게 했고, 성능상 매번 다시 그러주는 리소스가 줄어들기 때문에 좋습니다.

## 회고록

1. 개발하면서 느낀점이 볼륨이 작아서 계획했던 것대로 진행하면 오버워크가 될 것이란 생각이 듬.
    - 커스텀 훅으로 예정했던 init, supply, handler 는 취소하기로 결정. 페이지가 로드되는 직후에 api를 페칭하는 단계가 없어서 init이 무의미하고, suppler 메인 컨테이너가 페이지랑 모달에 딱 2개라서 중복 로직 거의 없음. 왠만한 상태관리 로직은 zustand내부에서 처리하고 있어서, 이상태에서 굳이 커스텀 훅으로 분리될 필요성이 없음. handler는 겹치는게 약간 있지만 1~2개 정도라 의미가 없음. 그런 고로 방치.
2. 다른 리소스 추출도 귀찮긴 마찬가지였지만, 팔레트가 없는게 제일 힘들게 여겨졌습니다. 알맞는 색깔이 없으니까 계속 바꾸게 되고, 대조해가면 다시 수정하게 되서. 역시 디자이너 이래서 필요하다고 다시 한번 느꼈습니다.
3. token store에 prefetch하는 부분이 중복로직이라서 유틸훅으로 뺄 수 있을 거 같은데. 밤을 세서 그런지 머리가 안돌아가서 이부분은 좀 덮어두겠습니다. 괜히 지금 시점에서 손댔다가 일이 커져서 시간이 부족할지도 모르겠습니다.
